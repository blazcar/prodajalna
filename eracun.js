  ...
  form.parse(zahteva, function(napaka1, polja, datoteke){
    pesmiIzRacuna(polja.seznamRacunov, function(pesmi){
      strankaIzRacuna(polja.seznamRacunov, function(stranka){
        ...
        odgovor.render('eslog', {vizualiziraj: true, postavkeRacuna: pesmi, strankica: stranka[0]});
      })
    })
  });
})
...
var kateraStrankaJeIzbrana = function(id, callback){
  ...
  //console.log(zahteva.session.Customer.FirstName);
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    ...
    try {
      ...
      stmt.run(polja.FirstName, polja.LastName, polja.Company, polja.Address, polja.City, 
               polja.State, polja.Country, polja.PostalCode, polja.Phone, polja.Fax,
               polja.Email, 3);
      stmt.finalize();
      prvic = false;
      odgovor.redirect("/prijava");
...
streznik.get('/prijava', function(zahteva, odgovor) {
  vrniStranke(function(napaka1, stranke) {
      vrniRacune(function(napaka2, racuni) {
        var uspesna = "Stranka je bila uspešno registrirana.";
        var neuspesna = "Prišlo je do napake pri registraciji nove stranke. Prosim preverite vnešene podatke in poskusite znova.";
        var status = "";
        if(napaka1){
          status = neuspesna;
        }
        ...
        if(!(napaka1 || napaka2 ) && prvic){
          status = uspesna;
        }
        odgovor.render('prijava', {sporocilo: status, seznamStrank: stranke, seznamRacunov: racuni});  
      }) 
    });
})
